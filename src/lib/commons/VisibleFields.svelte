	...
	<DropdownMenu style="max-height: 50vh; overflow-y: auto;">
		{#each allFieldNames.filter((name) => name !== '') as fieldName}
			{#if fieldsSelected.includes(fieldName)}
				<DropdownItem
					on:click={(e) =>
						(fieldsSelected = fieldsSelected.filter((val) => val !== e.target.innerText.slice(2)))}
				>
					✓ {fieldName}</DropdownItem
				>
			{:else}
				<DropdownItem on:click={(e) => (fieldsSelected = [...fieldsSelected, e.target.innerText])}
					>{fieldName}</DropdownItem
				>
			{/if}
		{/each}
