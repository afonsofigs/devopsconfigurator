<script>
	...
	import { ChatFieldsDefault, selfHosted } from '$lib/commons/options.js';
	...
	(() => {
		...
		auxFields.forEach((field) =>
			ChatFieldsDefault.includes(field.id)
	...
	function filteredData() {
		...
		function calcUsersBasedValues(arr) {
			...
			if (arr['FreeTemporaryGuestsFormula'] === '∞' || !isNaN(arr['FreeTemporaryGuestsFormula'])) {
				...
				arr['FreeTemporaryGuestsFinal'] = evaluate(
