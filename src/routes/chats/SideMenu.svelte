...
<Offcanvas header='More filters' isOpen={sideMenuOpen} placement='end' toggle={toggleEnd}>
	<div class='d-flex flex-column justify-content-start gap-4'>
		...
		<NumberInput bind:value={chatFiles} classNames='bg-light' label='Chat files GB' placeholder='GB' width='60px' />

		<NumberInput bind:value={freeGuests} classNames='bg-light' label='Free Guests' placeholder='guests' width='60px' />
