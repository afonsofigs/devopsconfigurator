<script>
  ...
  function filteredData() {
    ...
    function checkGitPlatforms(arr) {
      return (gitPlatformsSelected.length === 0 || arr["GitPlatformsCompatible"] === "All" ||
    ...
    function checkCloudBuildOSs(arr) {
      return (cloudOSsSelected.length === 0 ||
        (boolCloudBuild === "Or" ? cloudOSsSelected.some((os) => arr["CloudBuildOSs"].split(", ").includes(os))
    ...
    function checkConcurrentBuilds(arr) {
      return (arr["NCloudConcurrentBuilds"] === "∞" || concurrentBuilds <= parseInt(arr["NCloudConcurrentBuilds"]) ||
        arr["PriceExtraParallelBuild"] !== "N/A");
    }

    function checkSelfHostedRunners(arr) {
      return (arr["NSelf-hostedRunners"] === "∞" || selfHostedRunners <= parseInt(arr["NSelf-hostedRunners"]) ||
    ...
    function calcCustomPrice(arr) {
      ...
      arr["CalculatedCost"] = Math.round(
        (parseFloat(arr["LicenseCost$PerMonth"]) + priceUsers() + priceCloudConcurrentBuilds() +
          priceSelfHostedRunners() + priceCloudBuildMinutes("Linux", linuxMonthlyCloudMins) +
          priceCloudBuildMinutes("Docker", dockerMonthlyCloudMins) +
          priceCloudBuildMinutes("Windows", windowsMonthlyCloudMins) +
          priceCloudBuildMinutes("macOS", macOSMonthlyCloudMins) + Number.EPSILON) * 100) / 100;
    }
    ...
    filteredEntries = ciEntries
      .filter((arr) => {
        return (checkBrand(arr) && checkGitPlatforms(arr) && checkCloudBuildOSs(arr) && checkSelfHostedBuildOSs(arr) &&
          ...
          checkCloudBuilds(arr, "macOS", macOSMonthlyCloudMins) && checkSupport(arr) && checkCaching(arr) &&
          checkStats(arr) && checkScheduled(arr));
      })
      .map((arr) => {
        calcCustomPrice(arr);
  ...
  $: (ciEntries, cdChecked, selfHostedChecked, gitPlatformsSelected, users, concurrentBuilds, selfHostedRunners, linuxMonthlyCloudMins, dockerMonthlyCloudMins, windowsMonthlyCloudMins, macOSMonthlyCloudMins, brandsSelected, cloudOSsSelected, selfHostedOSsSelected, boolCloudBuild, boolSelfHostedBuild, supportSelected, statisticsChecked, scheduledChecked, cachingChecked), filteredData();
...
<div class="d-flex flex-row flex-wrap gap-4 justify-content-center align-content-center mb-3 mt-4">
  ...
  <div class="d-flex flex-row gap-1">
    <Label class="my-auto" for="runnersInput">CD</Label> <Input bind:checked={cdChecked} class="my-auto" id="s2"
                                                                type="switch" />
  </div>

  <MultiSelect bind:selected={gitPlatformsSelected} options={gitPlatforms} outerDivClass="bg-light"
...
<SideMenu bind:allBrands bind:boolCloudBuild bind:boolSelfHostedBuild bind:brandsSelected bind:cachingChecked
          bind:cloudOSsSelected bind:concurrentBuilds bind:currentPagination bind:dockerMonthlyCloudMins
          bind:linuxMonthlyCloudMins bind:macOSMonthlyCloudMins bind:scheduledChecked bind:selfHostedOSsSelected
          bind:selfHostedRunners bind:sideMenuOpen bind:statisticsChecked bind:supportSelected
