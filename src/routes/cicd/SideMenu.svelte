<script>
  ...
  export let cachingChecked = false;
  export let scheduledChecked = false;
  export let statisticsChecked = false;
</script>
...
<Offcanvas header="More filters" isOpen={sideMenuOpen} placement="end" toggle={toggleEnd}>
  <div class="d-flex flex-column justify-content-start gap-4">
    ...
    <GreenSwitch bind:checkVar={cachingChecked} label="Pipeline Caching" />
    <GreenSwitch bind:checkVar={statisticsChecked} label="Statistics and Metrics" />
    <GreenSwitch bind:checkVar={scheduledChecked} label="Scheduled Pipelines" />
