<script>
	...
	function getNBrands(entries) {
		...
		entries.forEach((entryArr) =>
			auxAllBrands.includes(entryArr['Brand'])
				? null
	...
	function allHighValuesPercentage(field, entries, totalBrands) {
		...
		entries.forEach((entryArr) => {
			...
			if (
				(isNaN(onAux) &&
					isNaN(newVal) &&
					onAux.includes('users') &&
					newVal.includes('users') &&
					evaluate(onAux.replace('users', 1)) < evaluate(newVal.replace('users', 1))) ||
				(isNaN(onAux) &&
					!isNaN(newVal) &&
					onAux.includes('users') &&
					newVal > evaluate(onAux.replace('users', 1))) ||
				(!isNaN(onAux) &&
					isNaN(newVal) &&
					newVal.includes('users') &&
					onAux < evaluate(newVal.replace('users', 1)))
			) {
				auxAllBrands[entryArr['Brand']] = newVal;
			}
		});
		...
		for (const value of Object.values(auxAllBrands)) {
			if (grouped[value] === undefined) grouped[value] = 1;
			else grouped[value] += 1;
		}
						...
						value: Math.round(
							entries.reduce((acc, arr) => {
								...
								function finalPrice() {
									return (
										Math.round(
											(parseFloat(arr['LicenseCost$PerMonth']) + priceUsers() + Number.EPSILON) *
												100
										) / 100
									);
								}
...
<div style="margin-top: 75px!important;">
	...
	<div class="mt-4 d-flex flex-row gap-5 flex-wrap justify-content-evenly">
		<Lines
			data={pricePerUser(100, vcsEntries, 2)}
			offsetBy="1"
			title="Average cloud-based monthly price per nº of users"
			xLegend="Nº Users"
		/>
		<Donut
			data={allHighValuesPercentage('Self-hosted', vcsEntries, vcsBrands)}
			offsetBy="1"
			title="Brands that offer Self-hosted solutions"
		/>
		<Donut
			data={allHighValuesPercentage('Issues', vcsEntries, vcsBrands)}
			offsetBy="1"
			title="Brands that offer Issue tracking"
		/>
		<Donut
			data={allHighValuesPercentage('Kanban', vcsEntries, vcsBrands)}
			offsetBy="1"
			title="Brands that offer Kanban boards"
		/>
		<Donut
			data={allHighValuesPercentage('Wiki', vcsEntries, vcsBrands)}
			offsetBy="1"
			title="Brands that offer Wiki documentation"
		/>
		<Donut
			data={allHighValuesPercentage('PackageRegistry', vcsEntries, vcsBrands)}
			offsetBy="1"
			title="Brands that offer Package Registry"
		/>
		<Bars
			data={allHighValuesPercentage('NPrivateReposFormula', vcsEntries, vcsBrands)}
			offsetBy="1"
			title="Brand's Nº Included Repos"
			xLegend="Nº Private Repos"
		/>
		<Bars
			data={allHighValuesPercentage('DiskSpaceGBFormula', vcsEntries, vcsBrands)}
			offsetBy="1"
			title="Brand's base disk GB included"
			xLegend="Disk GB"
		/>
		<Donut
			data={allHighValuesPercentage('CommercialSupport', vcsEntries, vcsBrands)}
			offsetBy="1"
...
<div class="mt-5">
	...
	<div class="mt-4 d-flex flex-row flex-wrap gap-5 justify-content-evenly">
		<Lines
			data={pricePerUser(100, cisEntries, 2)}
			offsetBy="0"
			title="Average cloud-based monthly price per nº of users"
			xLegend="Nº Users"
		/>
		<Donut
			data={allHighValuesPercentage('Self-hosted', cisEntries, cisBrands)}
			offsetBy="0"
			title="Brands that offer Self-hosted solutions"
		/>
		<Donut
			data={allHighValuesPercentage('CD', cisEntries, cisBrands)}
			offsetBy="0"
			title="Brands that offer CD solutions"
		/>
		<Bars
			data={splitArrayPercentages('GitPlatformsCompatible', cisEntries, cisBrands)}
			offsetBy="0"
			title="Git platforms by Brands"
			xLegend="Git platforms"
		/>
		<Bars
			data={splitArrayPercentages('CloudBuildOSs', cisEntries, cisBrands)}
			offsetBy="0"
			title="Cloud Build OSs by Brands"
			xLegend="Cloud Build OSs"
		/>
		<Bars
			data={splitArrayPercentages('Self-hostedRunnersBuildOSs', cisEntries, cisBrands)}
			offsetBy="0"
			step="1"
			title="Self-hosted Build OSs by Brands"
			xLegend="Self-hosted OSs"
		/>
		<Donut
			data={allHighValuesPercentage('CachingPipelineAndDependencies', cisEntries, cisBrands)}
			offsetBy="0"
			title="Brands that offer Caching in the pipeline"
		/>
		<Donut
			data={allHighValuesPercentage('ScheduledPipelines', cisEntries, cisBrands)}
			offsetBy="0"
			title="Brands that offer Scheduled Pipelines"
		/>
		<Donut
			data={allHighValuesPercentage('StatisticsAndMetrics', cisEntries, cisBrands)}
			offsetBy="0"
			title="Brands that offer Statistics and Metrics"
		/>
		<Bars
			data={allHighValuesPercentage('CommercialSupport', cisEntries, cisBrands)}
			offsetBy="0"
			title="Brands with Commercial Support"
		/>
	</div>
...
<div class="mt-5">
	...
	<div class="mt-4 d-flex flex-row flex-wrap gap-5 justify-content-evenly">
		<Lines
			data={pricePerUser(100, chatsEntries, 2)}
			offsetBy="2"
			title="Average cloud-based monthly price per nº of users"
			xLegend="Nº Users"
		/>
		<Bars
			data={allHighValuesPercentage('PeoplePerCall', chatsEntries, chatsBrands)}
			offsetBy="2"
			title="Brand's maximum users per call"
			xLegend="Users"
		/>
		<Donut
			data={allHighValuesPercentage('MsgHistory', chatsEntries, chatsBrands)}
			offsetBy="2"
			title="Brand's maximum chat history"
		/>
		<Donut
			data={allHighValuesPercentage('Integrations', chatsEntries, chatsBrands)}
			offsetBy="2"
			title="Brands that have Integrations"
		/>
		<Bars
			data={allHighValuesPercentage('CallDurationMins', chatsEntries, chatsBrands)}
			offsetBy="2"
			step="2"
			title="Brand's maximum call duration minutes"
			xLegend="Call minutes"
		/>
		<Bars
			data={allHighValuesPercentage('FreeTemporaryGuestsFormula', chatsEntries, chatsBrands)}
			offsetBy="2"
			title="Brand's maximum free temporary guests"
			xLegend="Nº Guests"
		/>

		<Bars
			data={allHighValuesPercentage('ChatFilesFormulaGB', chatsEntries, chatsBrands)}
			offsetBy="2"
			title="Brand's maximum chat files space"
			xLegend="GB"
		/>

		<Donut
			data={allHighValuesPercentage('Self-hosted', chatsEntries, chatsBrands)}
			offsetBy="2"
			title="Brands with Self-hosted solutions"
		/>
		<Donut
			data={allHighValuesPercentage('CommercialSupport', chatsEntries, chatsBrands)}
			offsetBy="2"
			title="Brands with Commercial Support"
		/>
	</div>
